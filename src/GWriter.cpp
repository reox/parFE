...
int C_ASCII_GWriter::Write(const int& i)
{
  return fprintf(file, "%d ", i);
}
...
HDF5_GWriter::HDF5_GWriter(MPI_Comm a_comm)
    : file(-1), groupname("/"), comm(a_comm), plist(H5Pcreate(H5P_FILE_ACCESS)), index(0), mpi_size(0), mpi_rank(0), file_info(MPI_INFO_NULL)
{
#ifdef HAVE_MPI
  MPI_Comm_size(comm, &mpi_size);
  MPI_Comm_rank(comm, &mpi_rank);
...
HDF5_GWriter::HDF5_GWriter(const std::string& a_filename, MPI_Comm a_comm)
    : file(-1), filename(a_filename), groupname("/"), comm(a_comm), plist(H5Pcreate(H5P_FILE_ACCESS)), index(0), mpi_size(0), mpi_rank(0), file_info(MPI_INFO_NULL)
{
#ifdef HAVE_MPI
  MPI_Comm_size(comm, &mpi_size);
  MPI_Comm_rank(comm, &mpi_rank);
...
bool HDF5_GWriter::Select(const std::string& s)
{
  ...
  //Test group
  temp = group;
  group = H5Gopen(temp, groupname.c_str());
  if (group < 0) {
    group = H5Gcreate(temp, groupname.c_str(),0);
  }
  H5Gclose(temp);
#endif
  return (group >= 0);
...
int HDF5_GWriter::Write(const std::string& name, hid_t type, const void* data, int num_global_elems, int num_my_elems, int elem_size, int my_offset)
{
